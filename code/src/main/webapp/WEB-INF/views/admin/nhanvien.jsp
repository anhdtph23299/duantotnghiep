...
<script>
                                    ...
                                    <div class="card-body pb-0">
                                           <div class="d-flex mb-3 align-items-center">
                                                    <div class="dz-media mr-3 rounded-circle">
                                                     ...
                                                     <span class="" style="color: #eb8153">\${item.chucVu.tenCV}</span>
                                                     </div>
                                             </div>
                                            <div class="mb-2">
                                                <span>Code: \${item.maNV}</span>
                                            </div>
                                            <span>Email: \${item.email}</span>

                                            <div class="mt-2">
                                                <span>Phone: \${item.sdt}</span>
                                            </div>
                                            <div class="mt-2">
                                              <span>Status: \${item.trangThai == 1 ? "Hoạt động": "Vô hiệu hóa"}</span>
                                            </div>
                                            <div class="d-flex justify-content-between my-3">
                                                <button class="btn btn-success" title="Update status" onclick="updateStatus(\${item.id}, \${item.trangThai})" >Click</button class="btn btn-success">
                                                 <a class="btn btn-secondary"  href="/admin/detailNV/\${item.id}" style="text-decoration: none;color: white;">Chi tiết</a>
                                            </div>

                                    </div>
                                    <div class="card-footer d-flex justify-content-between">
                                            <a class="edit-button"  href="/admin/updateNV/\${item.id}" style="text-decoration: none">
                                             ...
                                             <button type="button" class="delete-button" onclick="Delete(\${item.id})">
    ...
    function Delete(id) {
        ...
        showConfirm("Bạn có muốn xóa?", id)
            .then((confirmed) => {
                if (confirmed) {
                    $.ajax({
                        url: '/api/admin/nhanvien/delete/' + id,
                        method: 'DELETE',
    ...
    function updateStatus(id) {
        $.ajax({
            url: '/api/admin/nhanvien/updateStatus/' + id,
            method: 'PUT',
    ...
    function searchAll() {
        var maNV= $('#searchAll').val();
        var tenNV= $('#searchAll').val();
        ...
        $.ajax({
            url: '/api/admin/nhanvien/search',
            type: 'GET',
            data: searchNV,
            success: function (response) {
                if (response.success) {
                    var nhanviens = response.data;

                    var nhanVien = $('#cardNhanVien ');
                    nhanVien.empty();

                    nhanviens.forEach(function (item) {
                        var card = ` <div class="col-xl-4 col-xxl-4 col-sm-6">
                                <div class="card user-card">
                                    <div class="card-body pb-0">
                                           <div class="d-flex mb-3 align-items-center">
                                                    <div class="dz-media mr-3 rounded-circle">
