...
public class BienTheServiceImpl implements IBienTheService{
	...
	public List<BienTheDTO> save(List<BienTheDTO> bienThesDTO) {
		for (BienTheDTO bienTheDTO : bienThesDTO) {
			if(bienTheDTO.getHinhanhbase64() != null && bienTheDTO.getHinhanh() != null){
				...
				SanPhamHinhAnh sanPhamHinhAnh = bienTheConverter.convertToSanPhamHinhAnhEntity(bienTheDTO);
				sanPhamHinhAnhRepository.save(sanPhamHinhAnh);
			}

			BienThe bienThe = bienTheConverter.convertToEntity(bienTheDTO);
			bienTheRepository.save(bienThe);
			bienTheDTO.setId(bienThe.getId());
		}
		return bienThesDTO;
	}
	...
	public BienTheDTO findByGiaTriThuocTinh(List<Long> giaTriThuocTinhsId) {
		...
		BienThe bienThe = bienTheRepository.findById(bienTheId).get();
		Integer lenOfServerResponse = giaTriThuocTinhsId.size();
		Integer lenOfAttribute = bienThe.getSanphams().getThuocTinhEntities().size();
		if(lenOfAttribute != lenOfServerResponse) {
			return null;
		}
		BienTheDTO bienTheDTO = bienTheConverter.convertToDTO(bienThe);
		return bienTheDTO;
	}
